...
export async function startContainer(Id: string): Promise<boolean> {
  const result = await client.mutate<{ startContainer: boolean }>({
    mutation: gql`mutation { startContainer(Id: "${Id}") }`
  });
...
export async function createContainer(
  ...
  const result = await client.mutate<
    ...
    { env: DockerEnvironment[] }
  >({
    mutation: gql`mutation createContainer($env: [DockerEnvironment!]!){ createContainer(image: "${image}", env: $env) { id } }`,
    variables: { env }
  });
...
export async function removeContainer(Id: string): Promise<boolean> {
  const result = await client.mutate<{ removeContainer: boolean }>({
    mutation: gql`mutation { removeContainer(Id: "${Id}") }`
  });
  return result.data.removeContainer;
}

export async function pullImage(image: string): Promise<boolean> {
  const result = await client.mutate<{ pullImage: boolean }>({
    mutation: gql`mutation { pullImage(image: "${image}") }`
  });
